"""
Test cases for GoCardless webhook certificate validation
"""
import os
import unittest
import tempfile
from flask_backend.services.gocardless_service_updated import GoCardlessService

class GoCardlessWebhookTest(unittest.TestCase):
    """Test cases for GoCardless webhook certificate validation"""

    def setUp(self):
        """Set up test environment"""
        # Create temporary certificate files for testing
        self.temp_cert_file = tempfile.NamedTemporaryFile(delete=False)
        self.temp_key_file = tempfile.NamedTemporaryFile(delete=False)
        
        # Write test certificate data
        self.temp_cert_file.write(b"""-----BEGIN CERTIFICATE-----

9h9ULOCGKI/Y1U9D9MrKQSEj
-----END CERTIFICATE-----""")
        self.temp_cert_file.close()
        
        self.temp_key_file.write(b"""-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----""")
        self.temp_key_file.close()
        
        # Create a valid client certificate for testing
        self.valid_client_cert = """-----BEGIN CERTIFICATE-----

cO8VWkQA9o3zrwdJ
-----END CERTIFICATE-----"""
        
        # Invalid client cert (different issuer)
        self.invalid_client_cert = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""
        
        # Create the GoCardless service with sandbox mode off
        self.gocardless_service = GoCardlessService()
        self.gocardless_service.sandbox_mode = False
        self.gocardless_service.webhook_cert_path = self.temp_cert_file.name
        self.gocardless_service.webhook_key_path = self.temp_key_file.name

    def tearDown(self):
        """Clean up after tests"""
        os.unlink(self.temp_cert_file.name)
        os.unlink(self.temp_key_file.name)

    def test_no_certificate(self):
        """Test case with no certificate provided"""
        result = self.gocardless_service.verify_webhook_certificate(None)
        self.assertFalse(result)
        
        result = self.gocardless_service.verify_webhook_certificate("")
        self.assertFalse(result)

    def test_invalid_certificate_format(self):
        """Test case with invalid certificate format"""
        invalid_format = "This is not a valid certificate"
        result = self.gocardless_service.verify_webhook_certificate(invalid_format)
        self.assertFalse(result)

    def test_invalid_certificate_issuer(self):
        """Test case with invalid certificate issuer"""
        result = self.gocardless_service.verify_webhook_certificate(self.invalid_client_cert)
        self.assertFalse(result)

    def test_valid_certificate(self):
        """Test case with valid certificate"""
        # In sandbox mode, all certificates should be accepted
        self.gocardless_service.sandbox_mode = True
        result = self.gocardless_service.verify_webhook_certificate(self.valid_client_cert)
        self.assertTrue(result)
        
        # Reset to production mode for real validation
        self.gocardless_service.sandbox_mode = False
        # Note: In a real test environment, we'd need to generate properly signed certificates
        # for this test to pass with actual validation
        
if __name__ == '__main__':
    unittest.main()